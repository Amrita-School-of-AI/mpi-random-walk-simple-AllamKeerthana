...
void walker_process()
{
    // Seed the random number generator using unique seed per rank
    srand(static_cast<unsigned>(time(NULL)) ^ (world_rank << 8));
    ...
    for (steps = 0; steps < max_steps; steps++)
    {
        ...
        {
            steps++; // Include this step
            break;
        }
    }

    // Output walker result
...
void controller_process()
{
    ...
    // Final expected output (exact wording!)
